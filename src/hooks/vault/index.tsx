...
export default function SismoVaultProvider({
  ...
  const createSismoConnectDataSource = async ({
    vaultId,
    appId,
  }: {
    vaultId: string;
    ...
    if (
      ...
      vaultState.sismoConnectDataSources.find((el) => el.appId === appId && el.vaultId === vaultId)
    )
      return;
    const vault = await vaultClient.createSismoConnectDataSource({ vaultId, appId });
    await vaultState.updateVaultState(vault);
  };
