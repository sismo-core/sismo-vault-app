...
export class ZkConnectProver {
  ...
  public async generateResponse(
    ...
    const _zkConnectResponseBytes = await this.getZkConnectResponseBytes(
      zkConnectResponse
    );

    zkConnectResponse.zkConnectResponseBytes = _zkConnectResponseBytes;

    return zkConnectResponse;
  }

  private async getZkConnectResponseBytes(
      ...
      [
        {
          ...
          proofs: zkConnectResponse.proofs.map((proof) => {
            const claimForEncoding = {
              ...
              extraData: ethers.utils.toUtf8Bytes(
                proof?.claim?.extraData ?? ""
              ),
            } as Claim;
