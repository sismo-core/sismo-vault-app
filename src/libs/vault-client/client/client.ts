...
import { RecoveryKey, SismoConnectDataSource, Vault, VaultNamespaceInputs } from "./client.types";
...
export class VaultClient {
  ...
  public async getVaultId({
    ...
    const vaultNamespace = BigNumber.from(
      keccak256(
        ethers.utils.solidityPack(["uint128", "uint128"], [appId, BigNumber.from(derivationKey)])
      )
    )
      .mod(SNARK_FIELD)
      .toHexString();
  ...
  public async createSismoConnectDataSource({
    vaultId,
    appId,
  }: {
    vaultId: string;
    ...
    const sismoConnectDataSources = currentVault.sismoConnectDataSources
      ? [...currentVault.sismoConnectDataSources]
      : [];

    const vaultIdAlreadyExist = sismoConnectDataSources.find(
      (dataSource) => dataSource.vaultId === vaultId
    );
    if (vaultIdAlreadyExist)
      throw new Error("VaultId of Sismo Data Source already added in the Vault");

    const appIdAlreadyExist = sismoConnectDataSources.find(
      (dataSource) => dataSource.appId === appId
    );
    if (appIdAlreadyExist) throw new Error("AppId of Sismo Data Source already added in the Vault");

    const sismoConnectDataSource: SismoConnectDataSource = {
      vaultId: vaultId,
      appId: appId,
      createdAt: Date.now(),
    };
    ...
    const updatedVault: Vault = {
      ...currentVault,
      sismoConnectDataSources: [...sismoConnectDataSources, sismoConnectDataSource],
    };
